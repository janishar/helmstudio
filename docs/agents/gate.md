...
- **The samples.** Every code sample is a file under `site/samples`, and `site/gen/samples_test.go` names the test that runs each one. The quickstart runs as written, step by step, in a workspace of its own, with `helm dev` serving the example studio on port 8765 — the port the page names, so the test fails when it is taken. The Python samples run inside a studio under `helm dev`; the theming studio runs with its Python, Go and JavaScript servers; the Go embedded provider sample records with no daemon; the manifests validate. A step that needs the network — the quickstart's install of `helm`, its `pip install` and its download of the example — is not run, and the page says so beside it; the test checks that each names the installer, the package and the files this checkout has, and stands in for it from the checkout, with `helm` built from it on the path. A test fails when a sample is neither run nor marked.
