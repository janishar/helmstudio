...
| `helm-runtime-sdk` | Go module, PyPI package, npm package (with a browser ESM build).                              | Typed client for state, records, assets, gallery, jobs, timeline, events. Provider selection. | The OpenAPI contract. Optional embedded extra.      |
...
| Concern                 | How                                                                                                                                                                                                                                                                   |
|-------------------------|-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| Provider selection      | `fromEnv()` returns the remote client when `HELM_API` is set, the embedded one otherwise. Same interface either way.                                                                                                                                                  |
| Embedded stays optional | A separate module in Go (`helm-runtime-sdk/embedded`) and an extra in Python (`helm-runtime-sdk[embedded]`), so a hosted-only studio never pulls SQLite in. The Node package is remote-only; a Node studio running standalone points at a framework it starts itself. |
...
| Errors                  | One typed error shape across languages: `Conflict` (etag), `QuotaExceeded`, `NotFound`, `Unsupported` (an endpoint the current provider cannot serve), `Unavailable`. Components branch on these, so they must mean the same thing everywhere.                        |
| Testing                 | `helm dev` and the fixtures ship here, since they are the runtime's job. Every generated client runs the same conformance suite against both providers.                                                                                                               |
...
| Path                  | What it is                                                                                                                 | For                                                                                                                           |
|-----------------------|----------------------------------------------------------------------------------------------------------------------------|-------------------------------------------------------------------------------------------------------------------------------|
| Package registries    | `go get helmstudio.in/runtime`, `pip install helm-runtime-sdk`, `npm i @helmstudio/runtime @helmstudio/ui @helmstudio/css` | Studios with a toolchain, and CI.                                                                                             |
