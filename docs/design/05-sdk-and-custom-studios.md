...
Because the schema and the directory layout are identical, a studio that ran alone for months is *adopted* rather than migrated when helmstudio arrives: the daemon hardlinks the blobs and replays the rows. Keep the embedded provider a separate package — `helm-runtime-sdk/go/embedded` in Go — so a studio that only ever runs hosted never pulls SQLite into its dependency tree. (Amended 2026-09-15, M4, Q2: the names follow the package architecture document. Amended 2026-09-15, M5, Q14: there is no Python embedded provider; a Python studio runs standalone under `helm dev`.)
...
| `helm dev`                               | Runs the embedded provider behind the real HTTP surface on a local port and exports `HELM_API`, so the studio runs exactly as it would under helmstudio — same endpoints, same tokens, same capability scoping, no daemon. A studio that declares `python` runs in the author's own environment (the active `VIRTUAL_ENV`, or `-venv`), activated as the daemon activates its own; `helm dev` never creates or changes one (amended M5, Q15). |
