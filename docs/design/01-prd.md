...
- **R39** **Handoff.** `POST /handoff` sends an item to another studio's inbox; `GET /inbox` drains it. A studio that has not implemented an inbox receives the file in its stage directory instead.
