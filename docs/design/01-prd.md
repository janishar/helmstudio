...
- **R1** A studio's manifest — `helmstudio.yaml` — lives at the root of the studio's own repository, beside the code it describes. The bundled registry holds **pointers** (`id`, `repo`, `ref`), and the daemon resolves and caches the manifest from the repo at that ref. One copy, so nothing drifts, and a studio can announce a new build step without waiting for a helmstudio release. (Amended 2026-09-16, M7 Q4 and Q15: the pointer shape is `schema/registry-entry.json`, with an optional inline `manifest:` for a repository that ships none — one copy still, because the pull request that moves `ref` to a repository's own manifest removes the inline one. A pointer carries **no certification level**: a level is derived from what has been checked, never declared by the thing being checked.)
- **R2** Each manifest is validated at load; an invalid one is **listed as invalid**, with its source, its file and its errors, and offers Edit and Revert. It cannot be installed or launched. One bad manifest never blocks the others. (Amended 2026-09-16, M7 Q6: the original said an invalid manifest is not listed. Hiding it makes a registry studio vanish over a typo, with nothing on screen to explain where it went — and a user who broke their own override needs to find it in order to fix it.)
...
(Amended 2026-09-16, M8, Q4–Q19. The measurements these rest on are in `docs/agents/reports/08-timeline-and-export.md`.)
- **R44's document (Q7, Q8, Q11).** Times are seconds, snapped by the daemon to the target's frames — or to its samples for a clip on an audio track — so what is stored is what will be rendered, and writing a document back changes nothing. A sequence has one video track, contiguous from 0, and up to eight audio tracks; a video clip's own sound plays under it; an image is held rather than trimmed; the one transition is a dissolve centred on the cut, paid for with handles from both sides, which moves no clip. A sequence is owned by the studio that made it, or by the launcher; another studio reaches it only with `gallery.read_all`, and every clip must name an asset the caller may read, checked again at export.
- **R45's `:append` and `:open` (Q19, Q20).** `:append` takes a sequence id, and without one appends to the most recently updated sequence the caller owns. Until the launcher has a Timeline screen — which waits for the Mac app's cookie, because that screen reads every studio's items and bytes — `:open` answers 501 with a reason, which is what the design already says happens when no window is open.
- **R47's fast path (Q13).** The picture is copied; the sound is always re-encoded, because a copied AAC stream carries its own priming and padding into every cut — two real h3 takes joined that way put the second take's sound 81 ms behind its picture. So the chip reads "video stream copy", not "no re-encode". A copy is taken only when every clip, probed at export rather than trusted from what a studio said when it adopted it, matches the target and each other on codec, profile, level, size, aspect ratio, field order, pixel format, time base, frame rate, colour tags and parameter sets, and is used whole: a trim cuts at packets, not frames.
- **R48's item (Q17).** The export is the exporting studio's item, carrying `timeline_id` and one `clip` input per distinct asset. A live sequence holds its footage against reclaim; a deleted one does not.
- **R49's ffmpeg (Q4, Q5).** The licensing decision is closed: an LGPL build with videotoolbox. helmstudio names only `h264_videotoolbox`, ffmpeg's own AAC, stream copy and the uncompressed pair its probes and posters read through, and a test holds that line. Until the app bundles a pinned build (M9), ffmpeg is found through `HELM_FFMPEG` or the PATH; without one, the operations that need it answer 501 and everything else still works.
...
- **R62** Before anything executes, an approval screen shows the repo and ref, **every command verbatim**, the weights and sizes, the capabilities requested as plain sentences, the declared network hosts and the certification level. (Amended 2026-09-16, M7 Q11: "every build command" was too narrow. A manifest also runs `processes[].cmd` and `health.exec` at every launch and `import.run` once on first launch, so a harmless `build[]` with a hostile `cmd` would have passed a screen that showed build steps only. Commands are grouped by when they run, shown byte for byte, with control, zero-width and bidirectional characters escaped and flagged; submodule URLs are shown too.)
- **R63** Checks run before install: manifest validity, host requirements and capability review. Required failures warn loudly and block a registry merge; expected failures never block a user installing their own work. (Amended 2026-09-16, M7 Q13: theme conformance and the smoke harness are listed as **"Not run before install"**, with the reason, never as passes. A smoke test builds and runs the studio, which is the very thing this screen asks permission for, and theme conformance needs files that have not been cloned yet. A screen that ran them would have to ask permission to ask permission.)
- **R64** A studio carries a certification level shown as a card chip — Draft, Unverified, Verified, Registry — which labels but never gates what a user runs on their own machine. (Amended 2026-09-16, M7 Q15: the level is **derived**, never declared in a file. Until a smoke harness exists and its runs are recorded, only Draft — a manifest with `local_path` — and Unverified are reachable, so the four registry studios show source Registry and level Unverified. Claiming Verified without having verified anything is the one failure mode this label exists to prevent.)
...
|---------------------|-----------------------------------------------------------------------|--------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| ffmpeg licensing    | LGPL build with videotoolbox, or a GPL build with x264                | LGPL plus videotoolbox — also the fast encoder on Apple Silicon, and it avoids a GPL question in a signed MIT app. **Decided: the lean, closed in M8 (docs/decisions.md M8 Q4). helmstudio names only videotoolbox for h264 and ffmpeg's own AAC; a Linux build has no videotoolbox and needs another encoder, which is open.** |
