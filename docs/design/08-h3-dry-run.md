...
| `<session>/outputs/*.mp4` + `.json`  | most of the 55 MB | **Mirrored** to `assets` + `items`, with the sidecar becoming `params` and `refs[]` becoming `item_inputs`. Files stay where they are; adoption hardlinks.                                                                           |
