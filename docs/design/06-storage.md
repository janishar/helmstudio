...
|---------------------------|---------------------------------------------------------------------------------------------------------------------|--------------------------------------------------------------------------------------------------|-----------------------------------------------------------------------------------------------------------|
| `modernc.org/sqlite`      | Pure Go. `CGO_ENABLED=0`, so cross-compilation, CI and the app bundle stay trivial. Same SQL, same file format. | ~8–10 MB of binary, a slower cold build, and a large machine-translated codebase to trust.       | **Use this.** The workload is a few hundred writes an hour; raw driver speed is irrelevant.               |
...
|------------------------------------|-----------------------------------|----------------------------------------------------------------------------------------------------------------------------|
| Binary grows                       | ~8–10 MB with modernc             | Irrelevant next to a 16 GB checkpoint. (Amended 2026-09-18: this also said the Electron bundle dwarfs it, which a native shell does not — the app is now the daemon's size plus ffmpeg, not plus a Chromium. The first clause is the one that decides it.) |
